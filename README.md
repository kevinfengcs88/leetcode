...
So we define our HashMap, which will hold `<Integer, Integer>` key-value pairs. We loop through the items in `result` (the loop termination references `nums.length`, which is the same as `result.length`) and store `result[i], i` key-value pairs in the HashMap if `result[i]` does not already exist in the HashMap. Now we just have to take these HashMap values and place them in the correct indices of the array to be returned. Since we no longer need the sorted version of the `nums` array, and also because creating a new array would require additional memory allocation, we'll store the results in `result`. We loop through `result`, placing the value of the key `nums[i]` for each `result[i]`. This will place the correct values corresponding to the order of elements in the original `nums` array. Lastly, we return `result`. 